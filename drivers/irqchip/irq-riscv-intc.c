...
static bool __init acpi_validate_rintc_table(struct acpi_subtable_header *header,
                     ...
                     const unsigned long end)
{
    ...
    if(rintc->hartId.lo != smp_processor_id()) {
        return 0;
    }

    if(intc_domain) {
        pr_info("riscv_intc_acpi_init: RISCV INTC already initialized\n");
        return 0;
    }
