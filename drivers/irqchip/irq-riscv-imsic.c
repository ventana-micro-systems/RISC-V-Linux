...
struct imsic_handler {
	...
	/* IPI pending bits for given HART */
...
#ifdef CONFIG_SMP
static void imsic_ipi_dummy(struct irq_data *d)
{
}
...
static void imsic_ipi_send_mask(struct irq_data *d, const struct cpumask *mask)
{
	...
	for_each_cpu(cpu, mask) {
		...
		set_bit(d->hwirq, &handler->ipi_bits);
...
static struct irq_chip imsic_ipi_chip = {
	...
	.irq_mask	= imsic_ipi_dummy,
	.irq_unmask	= imsic_ipi_dummy,
...
static void imsic_ipi_process(struct imsic_priv *priv)
{
	int err;
	unsigned long irqs;
	irq_hw_number_t hwirq;
	...
	irqs = xchg(&handler->ipi_bits, 0);
	if (irqs) {
		for_each_set_bit(hwirq, &irqs, BITS_PER_LONG) {
			err = generic_handle_domain_irq(priv->ipi_domain,
							hwirq);
			if (unlikely(err))
				pr_warn_ratelimited(
...
static int __init imsic_ipi_set_virq(struct imsic_priv *priv)
{
	...
	ipi.fwnode	= priv->ipi_domain->fwnode;
	ipi.param_count	= 1;
	ipi.param[0]	= 0;
	virq = __irq_domain_alloc_irqs(priv->ipi_domain, -1, BITS_PER_LONG,
				       NUMA_NO_NODE, &ipi,
				       false, NULL);
	if (virq <= 0)
		return -ENOMEM;
...
static int __init imsic_ipi_domain_init(struct imsic_priv *priv)
{
	...
	priv->ipi_domain = irq_domain_add_linear(NULL, BITS_PER_LONG,
				  ...
				  const unsigned long end)
{
	...
	/* Compute base address */
	global->base_addr = ((u64)imsic->socket_imsic[0].imsic_addr_hi << 32) + imsic->socket_imsic[0].imsic_addr_lo;
	global->base_addr &= ~(BIT(global->guest_index_bits +
				 global->hart_index_bits +
				 IMSIC_MMIO_PAGE_SHIFT) - 1);
	global->base_addr &= ~((BIT(global->group_index_bits) - 1) <<
	...
	for (i = 0; i < nr_parent_irqs; i++) {
		...
		handler = per_cpu_ptr(&imsic_handlers, i);
		if (handler->priv) {
			pr_warn
