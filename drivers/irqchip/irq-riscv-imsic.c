...
static int __init imsic_ipi_domain_init(struct imsic_priv *priv)
{
	...
	/* Do nothing if IPI range not available */
	if (!priv->base_ipi || !priv->nr_ipis)
		return 0;

	/* Sanity check on IPI range */
	if (priv->nr_ids < priv->base_ipi)
		return -EINVAL;
	if (priv->nr_ids < (priv->base_ipi + priv->nr_ipis - 1))
		return -EINVAL;
	...
	priv->ipi_domain = irq_domain_add_linear(NULL,
						priv->nr_ipis,
			     ...
			     struct device_node *parent)
{
	...
	rc = of_property_read_u32(node, "riscv,num-ids", &priv->nr_ids);
	if (rc) {
...
#ifdef CONFIG_SMP
	/* Find IPI base and count */
	if (of_property_read_u32_index(node, "riscv,ipi-range", 0,
				       &priv->base_ipi))
		priv->base_ipi = 0;
	if (of_property_read_u32_index(node, "riscv,ipi-range", 1,
				       &priv->nr_ipis))
		priv->nr_ipis = 0;
#endif
