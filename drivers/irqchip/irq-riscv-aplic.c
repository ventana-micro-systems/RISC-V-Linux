...
static int aplic_setup_lmask_msis(struct aplic_priv *priv)
{
	...
	rc = of_property_read_u32(imsic_node, "riscv,guest-index-bits",
				  &mc->lhxs);
	if (rc) {
	...
	rc = of_property_read_u32(imsic_node, "riscv,hart-index-bits",
				  &mc->lhxw);
	if (rc) {
		/* Assume default value */
	...
	rc = of_property_read_u32(imsic_node, "riscv,group-index-bits",
				  &mc->hhxw);
	if (rc) {
		/* Assume default value */
		mc->hhxw = 0;
	...
	rc = of_property_read_u32(imsic_node, "riscv,group-index-shift",
				  &mc->hhxs);
	if (rc) {
		/* Assume default value */
