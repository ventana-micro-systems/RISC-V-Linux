...
static void aplic_msi_write_msg(struct msi_desc *desc, struct msi_msg *msg)
{
	...
	struct aplic_msi *msi = &priv->msis[desc->platform.msi_index + 1];
...
static int aplic_setup_lmask_msis(struct aplic_priv *priv)
{
	...
	/* Locate the IMSIC DT node */
	imsic_node = of_parse_phandle(node, "msi-parent", 0);
	if (!imsic_node) {
		dev_err(dev, "failed to find IMSIC DT node\n");
		return -ENODEV;
	}
	if (!of_device_is_compatible(imsic_node, "riscv,imsics")) {
		of_node_put(imsic_node);
		dev_err(dev, "compatible IMSIC DT node not found\n");
		return -ENODEV;
	}

	/* Find number of IMSIC parent interrupts */
	nr_parent_irqs = of_irq_count(imsic_node);
	if (!nr_parent_irqs) {
		of_node_put(imsic_node);
	...
	rc = of_property_read_u32(imsic_node, "imsic,guest-index-bits",
				  &mc->lhxs);
	if (rc) {
	...
	rc = of_property_read_u32(imsic_node, "imsic,hart-index-bits",
				  &mc->lhxw);
	if (rc) {
		/* Assume default value */
		mc->lhxw = __fls(nr_parent_irqs);
		if (BIT(mc->lhxw) < nr_parent_irqs)
			mc->lhxw++;
	}
	...
	rc = of_property_read_u32(imsic_node, "imsic,group-index-bits",
				  &mc->hhxw);
	if (rc) {
		/* Assume default value */
		mc->hhxw = 0;
	...
	rc = of_property_read_u32(imsic_node, "imsic,group-index-shift",
				  &mc->hhxs);
	if (rc) {
		/* Assume default value */
	...
	/* Get the first MMIO resource of IMSIC DT node */
	rc = of_address_to_resource(imsic_node, 0, &res);
	if (rc) {
		of_node_put(imsic_node);
		dev_err(dev, "IMSIC first MMIO address not found\n");
		return rc;
	}

	/* Compute PPN base */
	mc->base_ppn = res.start >> APLIC_xMSICFGADDR_PPN_SHIFT;
	...
	for (i = 0; i < nr_parent_irqs; i++) {
		if (of_irq_parse_one(imsic_node, i, &parent)) {
			dev_err(dev, "failed to parse IMSIC parent irq%d\n", i);
			return -EIO;
		}

		if (parent.args[0] != RV_IRQ_EXT) {
			dev_err(dev, "invalid hwirq for IMSIC parent irq%d\n",
				i);
			return -EIO;
		}
...
static int aplic_probe(struct platform_device *pdev)
{
	...
	of_property_read_u32(node, "aplic,num-sources", &priv->nr_irqs);
	if (!priv->nr_irqs) {
	...
	if (priv->nr_idcs) {
		...
		dev_dbg(dev, "MSI target: group index shift %d\n",
			priv->msicfg.hhxs + (2 * APLIC_xMSICFGADDR_PPN_SHIFT));
	}
...
static int aplic_acpi_probe(struct platform_device *pdev)
{
	...
	hart_index_bits.hart_index = aplic->hart_index;
	...
	aplic_acpi_data.imsic_info.guest_index_bits = hart_index_bits.lhxs;
	aplic_acpi_data.imsic_info.hart_index_bits = hart_index_bits.lhxw;
	aplic_acpi_data.imsic_info.group_index_bits = hart_index_bits.hhxw;
	aplic_acpi_data.imsic_info.group_index_shift = hart_index_bits.hhxs;
	aplic_acpi_data.num_harts = aplic->num_harts;
	aplic_acpi_data.base_cpu_id = aplic->base_cpu_id;
