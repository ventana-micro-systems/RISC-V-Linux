...
static u32 __imsic_swfile_topei(struct imsic *imsic)
{
	...
	max_msi = (imsic->nr_msis < imsic->swfile.eithreshold) ?
		  imsic->nr_msis : imsic->swfile.eithreshold;
	for (i = 1; i < max_msi; i++) {
		if (test_bit(i, imsic->swfile.eie) &&
		    test_bit(i, imsic->swfile.eip))
...
int kvm_riscv_vcpu_aia_imsic_update(struct kvm_vcpu *vcpu)
{
	...
	/* Get virtual and physical address of new IMSIC VS-file */
	new_hgei_va = imsic_cpu_page_virt(vcpu->cpu, new_hgei);
	ret = imsic_cpu_page_phys(vcpu->cpu, new_hgei, &new_hgei_pa);
	if (ret)
		goto fail_free_hgei;
