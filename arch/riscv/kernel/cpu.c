...
void acpi_print_hart_info(struct seq_file *m, struct acpi_table_header *table_hdr, unsigned long cpu)
{
	...
	while ((unsigned long)entry + entry->length <= table_end) {
		...
		if (acpi_cpu_id == entry->acpi_proc_id) {
			...
			/*
			extension_list = ACPI_ADD_PTR(struct acpi_rhct_hart_info_ext, 
					entry, sizeof(struct acpi_rhct_hart_info_cap));
			num_ext = (entry->length - sizeof(struct
					acpi_rhct_hart_info_cap)) / sizeof(struct
						acpi_rhct_hart_info_ext);
			acpi_print_extension(m, extension_list, num_ext);
